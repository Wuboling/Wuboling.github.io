...
public class MaxHeap {
    ...
    public int remove(){
       ...
       if(size==0) {
           ...
           while(t<half) {
               int child = 2*t+1;
               if(child+1<size && items[child+1]>items[child]) child++;

               if(key>=items[child]) break;
    ...
    public MaxHeap buildMaxHeap(int[] items) {
        MaxHeap heap = new MaxHeap(items.length);

        for(int i=0; i<items.length; i++) {
            if(i==0) {
                heap.items[size++] = items[i];
                continue;
            }
            ...
            while(t>0) {
                parent = (t-1)/2;
                if(heap.items[parent]>=key) break;

                heap.items[t] = heap.items[parent];
                t = parent;
            }
            heap.items[parent] = key;
            heap.size++;
        }
        return heap;
     }
